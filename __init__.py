...
def tagStats(cids, highlights=""):
    ...
    for nb, tag in l:
        ...
        percentOfCardsWithThisTagWhichAreSelected = str(round(
            (nb*100)/nbCardWithThisTag))+"%" if nbCardWithThisTag else "Error: no card with this tag in the fcollection."
...
def setupMenu(browser):
    ...
    a.setShortcut(QKeySequence(getUserOption(
        "Shortcut of 'Number of tags'", "Ctrl+t")))
    ...
    a.setShortcut(QKeySequence(getUserOption(
        "Shortcut of 'and highlight'", "Ctrl+shift+t")))
